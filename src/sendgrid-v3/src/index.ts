...
test('init handler adds rest of handlers', async function(t) {
  ...
  t.deepEqual(SendGrid.firstCall.args, [sendgridKey])
...
test('sendTemplate handler should build body and make request', async function(t) {
  ...
  t.deepEqual(seneca.act.firstCall.args, [{
    role: 'sendgrid',
  ...
  t.deepEqual(seneca.act.secondCall.args, [{
    role: 'sendgrid',
    cmd: 'makeRequest',
    body: seneca.act.returnValues[0],
  }])
})
...
test('makeRequest handler should create and post the email', async function(t) {
  ...
  t.deepEqual(emptyRequest.firstCall.args, [{method: 'POST', path: '/v3/mail/send', body}])
